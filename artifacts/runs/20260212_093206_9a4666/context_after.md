...
- Exported: **2026-02-12 09:32:52 UTC**
...
- Track fraud rate: `SUM(has_fraudulent_dispute) / COUNT(*) by merchant/month`
- Target fraud rate <3% for optimal fees; >8.3% triggers significant fee increases
...
- `acquirer_country`: Bank processing for merchant
- Comparing these reveals cross-border patterns and fraud indicators

## Critical Relationships

- **merchant** → merchant_data.json (account_type, MCC, capture_delay, acquirer)
- **acquirer_country** → acquirer_countries table (acquirer name → country mapping)
- **card_scheme + merchant attributes + transaction attributes** → fees.json (fee calculation)
- **MCC** (from merchant) → merchant_category_codes table (industry descriptions)

## Key Performance Indicators (KPIs)

1. **Authorization Rate**: Target >95% - percentage of non-refused transactions
2. **Fraud Rate**: Target <3% - ratio of fraudulent_dispute volume to total volume
...
### Description
Each row represents a merchant’s aggregated payment performance for a given calendar month (year, month), uniquely identified by (merchant, year, month). It includes total processed volume and transaction count, corresponding fraudulent volume and fraud transaction count, plus the computed fraud_rate (fraud_volume_eur / total_volume_eur). Also provides categorical segmentations for analysis and pricing/risk work: volume_tier (e.g., <100k, 100k–1m) and fraud_tier (e.g., <7.2%, 7.2%–7.7%, 7.7%–8.3%, >8.3%), useful for monthly trend monitoring, merchant benchmarking, and tier-based fee/risk reporting.
